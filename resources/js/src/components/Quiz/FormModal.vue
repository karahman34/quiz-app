...
export default {
  ...
  computed: {
    isEdit() {
      return Object.keys(this.quiz).length ? true : false;
    },
  },

  mounted() {
    if (this.isEdit) {
      const { text, image } = this.quiz;
  ...
  methods: {
    ...
    async create(payload) {
      try {
        const { data } = await axios.post("/quizzes", payload);
    ...
    async update(payload) {
      try {
        const { data } = await axios.post(`/quizzes/${this.quiz.id}`, payload);
